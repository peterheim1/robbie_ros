...
int runCommand() {
  ...
  case RIGHT_LIFT://c
    //arg1= arg1+offset;
    arg2 = map((arg1+offset), 130, -130, 0, 1023);
    Serial.println(arg2);
    ADC_SetPoint = arg2;
...
void loop() {
  ...
  mapped_Right_Lift = map((ADC_ServoPoti), 0, 1023, 130, -130)- offset;
  right_motor();
  Ser_print();
...
void receiveEvent (int howMany)
 ...
 if (howMany >= (sizeof Target))
   ...
   angle = Target;
   Target = map((Target + offset), 130, -130, 0, 1023);
   if (Target < MinVal){
    Target = MinVal;}
   if (Target > MaxVal){
    Target = MaxVal;}
