...
#define P_FRACTION 0.5        //0.0 - 10.0 (0.3)
#define I_FRACTION 0.3         //0.0 - 10.0 (0.3)
#define D_FRACTION 4.0         //0.0 - 10.0 (4.0)
#define V_WINDOW 10            //10 - 1000 (25) stop tollerance
...
#define SERVO_DIRECTION_PIN_2 7 //Direction pin servo motor
#define SERVO_PWM_PIN 9       //PWM pin servo motor
#define SERVO_SENSOR A0       //Analog input servo sensor
#define POTI A0               //Analog input potentiometer



/* Variable initialization */
float smoothedVal = 618;
float filterVal = 0.8; 
...
int runCommand() {
  ...
  case RIGHT_LIFT://c
   ...
   if (arg2 > MaxVal){
    ...
    break;
  case LEFT_LIFT://d
    Serial.print("48");// joint message left_tilt
    Serial.print("\t");
    Serial.print(mapped_Right_Lift);//tilt);//current position
    Serial.print("\t");
    Serial.print(ADC_ServoPoti);// joint target input
    Serial.print("\t");
...
*/
void loop() {
  ADC_ServoPoti = analogRead(POTI);     // reads the servo sensor (between 0 and 1023) 
  smoothedVal =  smooth(ADC_ServoPoti, filterVal, smoothedVal);
  mapped_Right_Lift = map((smoothedVal-offset), 0, 1023, -130, 130);
  right_motor();
  //Ser_print();
...
void sendSensor (const byte which)
  {
  int val = mapped_Right_Lift;//analogRead (which);
  byte buf [2];
