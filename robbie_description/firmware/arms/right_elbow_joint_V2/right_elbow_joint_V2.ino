...
#define P_FRACTION 1.5        //0.0 - 10.0 (0.3)
#define I_FRACTION 1.0         //0.0 - 10.0 (0.3)
#define D_FRACTION 4.0         //0.0 - 10.0 (4.0)
#define V_WINDOW 20            //10 - 1000 (25) stop tollerance
...
int runCommand() {
  ...
  case RIGHT_LIFT://c
    //arg1= arg1+offset;
    arg2 = map((arg1), 100, -100, 0, 1023) + offset;
    if (arg2 < MinVal){
    arg2 = MinVal;}
   if (arg2 > MaxVal){
    arg2 = MaxVal;}
...
void loop() {
  ...
  mapped_Right_Lift = map((ADC_ServoPoti + offset), 0, 1023, 100, -100)-31;
  right_motor();
  //Ser_print();
...
void receiveEvent (int howMany)
 ...
 if (howMany >= (sizeof Target))
   ...
   angle = Target;
   Target = map(Target, 100, -100, 0, 1023) + offset;//map((Target + offset), 130, -130, 0, 1023);
   if (Target < MinVal){
    Target = MinVal;}
   if (Target > MaxVal){
    Target = MaxVal;}
