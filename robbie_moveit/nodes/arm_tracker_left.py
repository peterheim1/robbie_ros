...
GRIPPER_FRAME = 'left_gripper_joint'

GRIPPER_OPEN = [0.1]
GRIPPER_CLOSED = [0.5]
GRIPPER_NEUTRAL = [0.0]

GRIPPER_JOINT_NAMES = ['left_arm_gripper_joint']
...
class ArmTracker:
    def __init__(self):
        rospy.init_node('left_arm_tracker')
        ...
        self.left_arm.set_goal_position_tolerance(0.1)#0.05
        
        # Set an orientation tolerance in radians
        self.left_arm.set_goal_orientation_tolerance(0.5)#0.2
        ...
        while not rospy.is_shutdown():
            ...
            target_arm = self.listener.transformPose('left_shoulder_tilt', target)
            ...
            if dist_target_shoulder > self.max_target_dist:
                ...
                self.left_arm.set_named_target('left_start')
                self.left_arm.go()
                rospy.sleep(1)
                continue
